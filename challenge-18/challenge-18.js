(function(){
    ...
    */
    function formCPF(cpf){
        return cpf.slice(0, 3) + '.' + cpf.slice(3, 6) + '.' + cpf.slice(6, 9) + '-' + cpf.slice(9, 11)
    }
    console.log( '\nFormatando CPFs corretamente:' );
    console.log(formCPF(cleanCPF('049-214 3421-1')))
    console.log(formCPF(cleanCPF('210.458.522-05')))
    console.log(formCPF(cleanCPF('735 500 794 - 22')))
    console.log(formCPF(cleanCPF('101.123-131x32')))
    ...
    */
    /*function extHtml(tag){
        var regxp = /<(\w+)>([a-zA-Záéíóúãõ\s]+)<\/\w+>/g
        return tag.replace(regxp, function(inutil ,atag , aval, btag, bval, ctag, cval){
            return '<' + atag +'>O texto dentro da tag "' + atag +'" é "' + aval +'"</'+ atag + '>', '<' + btag +'>O texto dentro da tag "' + btag +'" é "' + bval +'"</'+ btag + '>', '<' + ctag +'>O texto dentro da tag "' + ctag +'" é "' + cval +'"</'+ ctag + '>';
        })
    }
    console.log( '\nFazer replace dos textos das tags:' );
    console.log(extHtml('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'))*/
})();
